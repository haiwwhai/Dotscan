...
export default async (provider) => {
  ...
  try {
    ...
    const session_index = await api.query.session.currentIndex((header) => {
      var temp = header;
      createElement(` ${header} `  , div_session_index);     
    ...
    const session_len = await api.query.session.sessionLength((header) => {
     ...
     const era_index = await api.query.staking.currentEra((header) => {
      var temp = header;
      createElement(` ${header} `  , div_era_index);     
      var a=document.getElementById("era_index").innerText;
      if (temp!=a){        
        document.getElementById("era_index").innerText=temp;
      };     
     });
